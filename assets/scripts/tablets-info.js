// Tablets Info Page - Fetch and Display Product Details
...
class TabletsInfoPage {
    ...
    async init() {
        ...
        if (!this.productId) {
            this.showErrorState('No product ID provided. Please select a product from the tablets page.');
            return;
        }
    ...
    async fetchProductData() {
        try {
            ...
            // Fetch all products from the API (matching tablets.js pattern)
            const url = `${API_CONFIG.BASE_URL}${API_CONFIG.LIST_PRODUCTS_ENDPOINT}?category=tablets`;
            ...
            // Parse response - handle different response structures (matching tablets.js)
            let productsData = data;
            if (data.body) {
                productsData = typeof data.body === 'string' ? JSON.parse(data.body) : data.body;
            }

            // Extract products array from response (matching tablets.js)
    ...
    displayColorButtons() {
        ...
        // Get colors for this model from tablets_info-color.js (if it exists)
    ...
    displayStorageButtons() {
        ...
        // Get storage options for this model from tablets_info-color.js (if it exists)
    ...
    setupEventListeners() {
        ...
        if (compareBtn) {
            compareBtn.addEventListener('click', (e) => {
                ...
                const category = 'tablets'; // Since this is tablets-info page
            ...
            main.innerHTML = `
                <div class="error-state">
                    <div class="alert alert-danger text-center">
                        ...
                        <a href="tablets.html" class="btn btn-primary">Back to Tablets</a>
                    </div>
                </div>
            `;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check if we're on the tablet-info.html or tablets-info.html page
    if (window.location.pathname.includes('tablet-info.html') || window.location.pathname.includes('tablets-info.html')) {
        // Initialize tablets info page
        window.tabletsInfoPage = new TabletsInfoPage();
    }
});
