// Gaming Info Page - Fetch and Display Product Details
// API Configuration - matching gaming.js (use existing if already declared)
...
class GamingInfoPage {
    constructor() {
        this.productId = null;
        this.productData = null;
        this.priceHistoryChart = null;
        this.category = 'gaming-consoles';
        this.init();
    }
    ...
    async init() {
        ...
        if (!this.productId) {
            this.showErrorState('No product ID provided. Please select a product from the gaming page.');
            return;
        }
    ...
    async fetchProductData() {
        try {
            ...
            // Determine which category to fetch from based on product data or try all gaming categories
            const categories = ['gaming-consoles', 'gaming-laptops', 'gaming-monitors'];
            ...
            for (const category of categories) {
                try {
                    ...
                    console.warn(`Error fetching from category ${category}:`, error);
                    continue; // Try next category
                }
            }
            
            if (!productFound || !this.productData) {
                throw new Error(`Product with ID "${this.productId}" not found in any gaming category`);
            }
    ...
    updateBreadcrumb() {
        ...
        const brand = this.productData.brand || '';
        const fullProductName = brand ? `${brand} ${productName}`.trim() : productName;
        ...
        if (breadcrumb) {
            breadcrumb.textContent = fullProductName;
            breadcrumb.setAttribute('title', fullProductName);
        }

        // Update category breadcrumb based on gaming category
        if (categoryBreadcrumb && categoryBreadcrumbLink) {
            let categoryName = 'Gaming';
            let categoryLink = 'gaming.html?category=consoles';
            
            // Check URL parameter first (user's intent), then this.category, then productData.category
            const urlParams = new URLSearchParams(window.location.search);
            const urlCategory = urlParams.get('category');
            
            // Get the actual category - check multiple sources
            const detectedCategory = urlCategory || this.category || this.productData.category || '';
            
            // Normalize category string for comparison
            const categoryStr = String(detectedCategory).toLowerCase();
            
            // Check for Gaming Consoles
            if (categoryStr === 'consoles' || categoryStr.includes('gaming-consoles') || categoryStr.includes('consoles')) {
                categoryName = 'Gaming Consoles';
                categoryLink = 'gaming.html?category=consoles';
            } 
            // Check for Gaming Laptops
            else if (categoryStr === 'gaming-laptops' || categoryStr.includes('laptop-gaming') || categoryStr.includes('gaming-laptops')) {
                categoryName = 'Gaming Laptops';
                categoryLink = 'gaming.html?category=laptop-gaming';
            } 
            // Check for Gaming Monitors
            else if (categoryStr === 'gaming-monitors' || categoryStr.includes('monitors') || categoryStr.includes('gaming-monitors')) {
                categoryName = 'Gaming Monitors';
                categoryLink = 'gaming.html?category=gaming-monitors';
            }
            
            // Set the breadcrumb link - ensure it's properly set
            categoryBreadcrumbLink.textContent = categoryName;
            categoryBreadcrumbLink.removeAttribute('href'); // Remove old href first
            categoryBreadcrumbLink.setAttribute('href', categoryLink);
            categoryBreadcrumbLink.href = categoryLink;
            
            // Ensure the link is clickable (remove any pointer-events or disabled states)
            categoryBreadcrumbLink.style.pointerEvents = 'auto';
            categoryBreadcrumbLink.style.cursor = 'pointer';
        }

        // Also update page title dynamically
        if (document.title) {
            document.title = `${fullProductName} - CompareHubPrices`;
        }
    }
    ...
    extractKeySpecs(specs) {
        ...
        // Processor (for gaming laptops)
        if (specs.Performance?.Processor) {
            keySpecs.push({ label: 'Processor', value: specs.Performance.Processor });
        } else if (specs.Performance?.['Processor Type']) {
            keySpecs.push({ label: 'Processor', value: specs.Performance['Processor Type'] });
        }
        
        // RAM (for gaming laptops)
        ...
        // Operating System (for gaming laptops)
        if (specs.Os?.['Operating System']) {
            keySpecs.push({ label: 'OS', value: specs.Os['Operating System'] });
        }
        
        // Display/Screen Size (for gaming monitors and laptops)
        if (specs.Display?.Main?.Size) {
            keySpecs.push({ label: 'Display', value: specs.Display.Main.Size });
        } else if (specs.Display?.Size) {
            keySpecs.push({ label: 'Display', value: specs.Display.Size });
        } else if (specs.Display?.['Screen Size']) {
            keySpecs.push({ label: 'Screen Size', value: specs.Display['Screen Size'] });
        }
        
        // Graphics (for gaming laptops)
        if (specs.Performance?.Gpu) {
            keySpecs.push({ label: 'Graphics', value: specs.Performance.Gpu });
        }
        
        // Console-specific specs (for gaming consoles)
        if (specs.Console?.Generation) {
            keySpecs.push({ label: 'Generation', value: specs.Console.Generation });
        }
        if (specs.Console?.Storage) {
            keySpecs.push({ label: 'Storage', value: specs.Console.Storage });
        }
    ...
    displaySpecifications() {
        ...
        if (!specsTableBody) {
            console.warn('Specifications table body not found');
            return;
        }
    ...
    setupEventListeners() {
        ...
        if (compareBtn) {
            compareBtn.addEventListener('click', (e) => {
                ...
                // Determine category for comparison
                let category = 'gaming-consoles';
                if (this.category === 'gaming-laptops' || this.productData.category === 'gaming-laptops') {
                    category = 'gaming-laptops';
                } else if (this.category === 'gaming-monitors' || this.productData.category === 'gaming-monitors') {
                    category = 'gaming-monitors';
                } else if (this.category === 'gaming-consoles' || this.productData.category === 'gaming-consoles') {
                    category = 'gaming-consoles';
                }
                
                // Navigate to comparison page with product ID
                window.location.href = `comparison.html?products=${productId}&category=${category}`;
            });
        } else {
    ...
    async updatePriceHistoryChart(period = 30) {
        const priceChart = document.getElementById('priceHistoryChart');
            ...
            main.innerHTML = `
                <div class="error-state">
                    <div class="alert alert-danger text-center">
                        ...
                        <a href="gaming.html" class="btn btn-primary">Back to Gaming</a>
                    </div>
                </div>
            `;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check if we're on the gaming-info.html page
    if (window.location.pathname.includes('gaming-info.html')) {
        // Initialize gaming info page
        window.gamingInfoPage = new GamingInfoPage();
    }
});
